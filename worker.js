...
async function handleRequest(request) {
  try {
    const { message, chatId, businessConnectionId, senderId } =
      await parseTelegramRequest(request);
    if (message && message.text) {
      if (businessConnectionId) {
        ...
        if (message.text.startsWith("/ask")) {
          ...
          const systemPrompt =
            Systemprompt[senderId] ||
            "You are a friendly assistant. Available commands for this chat can be received by user on sending 'help'. ";
          ...
          await sendTelegramMessage(
            ...
            "Conversation history and system prompt cleared."
          );
        } else if (
          message.text.startsWith("/test") ||
   ...
   \`\`\``;
          ...
          message.text.toLowerCase() === "image"
        ) {
          ...
          if (response) {
            ...
            await sendTelegramMessage(
              chatId,
              businessConnectionId,
              "Error fetching NASA image."
            );
          }
        } else if (
          message.text.startsWith("/sticker") ||
          message.text.toLowerCase() === "sticker"
        ) {
          const protectContent = message.text.toLowerCase().includes("protect");
          ...
          message.text.startsWith("/gen") ||
          message.text.toLowerCase().startsWith("gen")
        ) {
          const prompt = message.text.startsWith("/gen")
            ? message.text.slice(4).trim()
            : message.text.trim();
          await generateImage(prompt, chatId, businessConnectionId, senderId);
        } else if (
          message.text.startsWith("/random") ||
          message.text.toLowerCase() === "random"
        ) {
          const protectContent = message.text.toLowerCase().includes("protect");
          ...
          const messageText = messageTextMatch
            ? messageTextMatch[1].trim()
            : "";

          const photoUrlMatch = text.match(/\[(.*?)\]/g);
          const photoUrl =
            photoUrlMatch && photoUrlMatch.length > 1
              ? photoUrlMatch[1].slice(1, -1).trim()
              : "";

          const buttonList = lines
            .slice(1)
          ...
          buttonList.forEach((button) => {
            ...
            if (buttonText && url) {
              inlineKeyboard.push([
                { text: buttonText.trim(), url: url.trim() },
              ]);
          ...
          if (messageText) {
            if (photoUrl.startsWith("http") || photoUrl.startsWith("https")) {
              await sendTelegramPhoto(
                ...
                messageText,
                false,
                hasValidButtons ? inlineKeyboard : null
              );
            } else {
              await sendTelegramMessage(
                chatId,
            ...
            await sendTelegramMessage(
              chatId,
              businessConnectionId,
              "No message text found. Check the format again."
            );
          }
          ...
          if (!hasValidButtons) {
            await sendTelegramMessage(
              chatId,
              businessConnectionId,
              "No valid button text/url found. Check the format again."
            );
          }
        ...
        // Handle user messages - no business connection id
        if (message.text.startsWith("/set")) {
          ...
          const defaultMessage =
            'This is primarily a business bot. Telegram premium users can set it as their chatbot. You can try it out on @blackarachnia. If you want to set a custom system prompt for the AI Model, send /set "your prompt".';
...
async function queryGroq(messages, systemPrompt) {
  ...
  return (
    data.choices[0]?.message?.content || "Invalid response from LLM endpoint"
  );
}
async function queryGrammarly(text) {
  try {
    const grammarlyApiUrl = `https://api.safone.dev/grammarly?text=${encodeURIComponent(
      text
    )}`;
...
async function getNasaImage() {
  try {
    ...
    const response = await fetch(
      `https://api.nasa.gov/planetary/apod?api_key=${apiKey}&date=${randomDate}&thumbs=true`
    );
...
async function sendTelegramDice(
  chatId,
  businessConnectionId,
  protectContent = false
) {
...
async function sendWikiResponse(
  chatId,
  businessConnectionId,
  text,
  originalUrl,
